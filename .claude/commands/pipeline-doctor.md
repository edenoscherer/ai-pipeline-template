...
`ARQUIVO_ARQUITETURA`, `ARQUIVO_QUALITY_GATES`, `ARQUIVO_ROADMAP`,
`ARQUIVO_DECISIONS_LOG`, `DOCS_FEATURES_DIR`).
...
5. **`ARQUIVO_QUALITY_GATES` existe e está preenchido**: abra o
   ...
   como não passou.
6. **`ARQUIVO_ROADMAP`** — se configurado (não vazio em `config.md`),
   confirme que o caminho existe. Se `config.md` deixou vazio de
   propósito, reporte `— (não usado neste projeto)`, não como falha.
7. **`ARQUIVO_DECISIONS_LOG`** — mesma regra do item 6.
8. **`DOCS_FEATURES_DIR`** — mesma regra do item 6 (confirma que o
   diretório existe, não que tem conteúdo).
9. **Comandos esperados existem em `.claude/commands/`** (ou
...
10. **Skills esperadas existem em `.claude/skills/`**:
    `clarification-protocol`, `software-dev-panel` — 2 no total.
11. **Para cada domínio em `DOCS_FEATURES_DIR`** (se configurado e o
...
12. **`.pipeline/version` existe e é legível** — reporte a versão
...
Pipeline: 75% saudável (9/12 verificações passaram)
```
